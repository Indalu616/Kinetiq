      ...
      >
        <div className="flex h-16 items-center gap-2.5 border-b border-sidebar-border px-5">
          ...
          <div className="leading-tight">
            <div className="text-[15px] font-semibold text-sidebar-ink">Kinetiq</div>
        ...
        <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4" aria-label="Primary">
          {NAV_ITEMS.map((item) => (
            <NavLink
              ...
              onClick={onCloseMobile}
              className={({ isActive }) =>
                `group relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-[13.5px] font-medium transition ${
                  isActive
            ...
            >
              {({ isActive }) => (
                <>
                  <span
                    className={`absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-full ${isActive ? 'bg-primary' : 'bg-transparent'}`}
                  />
